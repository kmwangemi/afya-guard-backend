...
async def seed_roles(db: AsyncSession, perm_map: dict[str, Permission]) -> None:
    ...
    for role_name, (display_name, description, is_system) in ROLES.items():
        ...
        if not role:
            ...
            db.add(role)
            await db.flush()
            # ✅ Re-fetch with selectinload after flush so permissions is initialized
            result = await db.execute(
                select(Role)
                .options(selectinload(Role.permissions))
                .filter(Role.name == role_name)
        ...
        # Now safe — permissions is eagerly loaded in both branches
...
async def seed_superuser(db: AsyncSession) -> None:
    """Create the default superuser if no superuser exists yet."""
    ...
    if existing_super:
        print(
            f"  {cyan('·')} Superuser already exists: {existing_super.email} — skipped"
        )
        return
    user = User(
    ...
    print(f"  {yellow('⚠')}  Change it immediately via PATCH /api/v1/auth/password")
