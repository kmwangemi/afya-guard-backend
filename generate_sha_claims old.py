...
FORM_FIELDS = {
    ...
    "admission_date": {"page": 2, "x": 180, "y": 130, "size": 9},
    ...
    "adm_date_row1": {"page": 2, "x": 83, "y": 654, "size": 7},
    "dis_date_row1": {"page": 2, "x": 142, "y": 654, "size": 7},
    "case_code_row1": {"page": 2, "x": 199, "y": 654, "size": 7},
    "icd_proc_row1": {"page": 2, "x": 241, "y": 654, "size": 7},
    "description_row1": {"page": 2, "x": 317, "y": 654, "size": 7},
    "preauth_row1": {"page": 2, "x": 397, "y": 654, "size": 7},
    "bill_amount_row1": {"page": 2, "x": 445, "y": 654, "size": 7},
    "claim_amount_row1": {"page": 2, "x": 492, "y": 654, "size": 7},
    # ── PAGE 3 ──────────────────────────────────────────────────────────────
    ...
    "patient_decl_name": {"page": 3, "x": 110, "y": 457, "size": 8},
    "declaration_date": {"page": 3, "x": 80, "y": 468, "size": 8},
}


def _draw_x_marker(c, x: float, y: float):
    """Draw an X mark at the given position (for checkboxes)."""
...
def fill_form(patient: dict, input_pdf: str, output_pdf: str) -> None:
    ...
    # Build one overlay canvas per page
    overlays = {}
    for pg_num in range(1, num_pages + 1):
        ...
        c.setFillColorRGB(0, 0, 0.6)  # dark blue ink — looks handwritten
    ...
    def write(field_key: str, value: str):
        ...
        x = f["x"]
        # PDF coordinate: y=0 at BOTTOM. Our coords are y=0 at TOP.
        y_pdf = page_h - f["y"]
        c.setFont("Helvetica", f.get("size", 9))
        c.drawString(x, y_pdf, str(value))

    def write_x(field_key: str):
        """Write an X marker at a field position."""
        if field_key not in FORM_FIELDS:
            return
        f = FORM_FIELDS[field_key]
        pg = f["page"]
    ...
    # Page 2
    # Visit type checkboxes: ☐Inpatient ☐Outpatient ☐ Day-care
    # Checkbox positions from form_structure: Inpatient box ~x=130, Outpatient ~x=208, Day-care ~x=285
    # Row y label top = 94; text baseline at y=98 centres inside the row
    vt = patient["visit_type"]
    c2, _, h2 = overlays[2]
    c2.setFont("Helvetica-Bold", 9)
    if vt == "Inpatient":
        c2.drawString(130, h2 - 98, "X")
    elif vt == "Outpatient":
        c2.drawString(208, h2 - 98, "X")
    else:
        c2.drawString(285, h2 - 98, "X")
    ...
    # Disposition checkbox — X placed inside the ☐ box for the correct option
    # Box positions from form_structure.json (y from top, x of checkbox):
    #   Improved ☐:          y=267, checkbox at ~x=196
    #   Recovered ☐:         y=288, checkbox at ~x=197
    #   Leave Against ☐:     y=310, checkbox at ~x=365
    #   Absconded ☐:         y=331, checkbox at ~x=201
    #   Died ☐:              y=353, checkbox at ~x=172
    dispo_map = {
        "Improved": (196, 271),
        "Recovered": (197, 292),
    ...
    # Benefits table row 1 — each column in its own cell
    ...
    write("description_row1", patient["description"][:14])  # max 14 chars to fit column
    ...
    # Page 3 — totals (align with Bill/Claim Amount columns)
    write("total_bill", f"{patient['total_bill_amount']:,}")
    write("total_claim", f"{patient['total_claim_amount']:,}")

    # Patient declaration — name written on the '___' line after 'Names (Majina):'
    # date written after 'Date(Tarehe):' on the same line
    write("patient_decl_name", patient["patient_name_declaration"])
    write("declaration_date", patient["declaration_date"])

    # ── Finalise canvases and merge with original pages ──────────────────────
    writer = PdfWriter()
